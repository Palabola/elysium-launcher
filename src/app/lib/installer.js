...
const Installer = function ( opts, electron ) {
    ...
    self.client.add(self._uri, {path: self._paths.download}, function (torrent) {
        ...
        electron.log('Downloading World of Warcraft 1.12');
        ...
        torrent.on('done', () => {
            ...
            self.client.remove(torrent, (err) => {
                ...
                self.emit('in-progress', `Extracting ${self._paths.file}`);
                electron.log( `Extracting ${self._paths.file}`);
                ...
                extract.on('exit', () => {
                    ...
                    self.emit('in-progress', `Installing World of Warcraft 1.12`);
                    electron.log( 'Installing World of Warcraft 1.12');

                    fs.move(path.join(self._paths.extract, "World of Warcraft 1.12"), self._paths.install, {clobber: true}, (err) => {
                        if (err) {
                            electron.err( err );
                        }
